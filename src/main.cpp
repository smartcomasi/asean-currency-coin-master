...
unsigned int nTargetSpacing     = 60;               // 30 seconds, FAST
...
const string strMessageMagic = "Aseancoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
	...
	if (pindexBest->nHeight == 1)
		nSubsidy = 10500249500 * COIN;  // 50% Premine	
	else if (pindexBest->nHeight <= FAIR_LAUNCH_BLOCK) // Block 500 Instamine prevention
        	nSubsidy = 1 * COIN;	 // 500 ACCOIN for instamine prevention
	else if (pindexBest->nHeight <= 21000000) // 10,500,000,000 ACCOIN 
		nSubsidy = 200 * COIN;		
    else if (pindexBest->nHeight > LAST_POW_BLOCK) // Block 3m
		nSubsidy = 0; // PoW Ends ~ 21,000,000,000 Total ACCOIN Mined via PoW
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    nRewardCoinYear = COIN_YEAR_REWARD; // 0.05 5%
...
//////////////////////////Comment out this shit if you wanna generate a new genesis block///////////////////////////////
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Aseancoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "28/JUN/2018 ACCOINCOIN LAUNCH SUPPORTED BY THE ACCOIN GROUP ";
        CTransaction txNew;
        txNew.nTime = 1529957172;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	...
	if(fTestNet)
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x307dd053f04c13d64bc130b8a3ce6d99de70468f612d28543f5942559a716d9d"));
        block.print();
